...
export const Testimonials: React.FC = () => {
  ...
  return (
    <section ref={sectionRef} className="py-24 bg-white dark:bg-swiss-black overflow-hidden transition-colors duration-300">
      <div className="container mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {TESTIMONIALS.map((t, index) => (
            <div 
              key={t.id} 
              className={`bg-slate-50 dark:bg-slate-900 p-8 rounded-3xl border border-slate-100 dark:border-slate-800 relative transition-all duration-700 ease-out ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}
